...
- front-vues 前端启动过程
  ...
  - yarn upgrade
  - yarn upgrade @achrinza/node-ipc
  - npm uninstall -g @achrinza/node-ipc
  - npm install @achrinza/node-ipc@latest
  - npm audit fix
  - yarn upgrade
  - npm install -g npm-check-updates (https://blog.csdn.net/weixin_45491473/article/details/108631023)
  - 之后即可启动
- IceCMS-front-admin 启动过程
  - npm install -g pnpm (安装pnpm)
  - pnpm install （安装依赖）
  - 之后即可启动


通过package.json中的dev启动不了
当前的npm版本过高
