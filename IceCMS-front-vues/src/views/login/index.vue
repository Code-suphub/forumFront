      ...
      <div class="registArea">
        ...
        <transition name="animate__animated animate__bounce" enter-active-class="animate__fadeInUp"
          ...
          <div v-show="!isShow" class="registForm">
            ...
            <div style="flex: 1;display: flex;justify-content: center;align-items: center">密{{ "\xa0" }}{{ "\xa0" }}{{ "\xa0" }}{{ "\xa0" }}{{ "\xa0" }}{{ "\xa0" }}{{ "\xa0" }}{{ "\xa0" }}{{ "\xa0" }}{{ "\xa0" }}码:
           ...
           <el-button v-show="!isShowyan" type="success"  >
             <div>  <a class="nav-link p-0 cursor-pointer"><i  class="icon-smile fs-18" /></a>
           <div>成功</div></div>
           </el-button>
           </div>
          </div>
...
export default {
  ...
  setTimeout(() => {
    ...
    UserLogin() {
      ...
      const that = this;
      this.loading = true
      login(that.loginUser).then(resp => {
        console.log(resp)
        if (resp.data.code === 402 || resp.data.code === 400) {
          ...
          setUser(resp)
          that.$router.push({ path: '/userinfo/index' }) // 跳到个人界面
          that.$message({
            message: '登陆成功',
            type: 'success'
          })
      ...
      //  }
        ...
        register(user).then(resp => {
          if (resp.data.code === 402 || resp.data.code === 400) {
            ...
            that.$router.push({ path: '/userinfo/index' }) // 跳到主页
            that.$message({
              message: '注册成功',
              type: 'success'
            })
