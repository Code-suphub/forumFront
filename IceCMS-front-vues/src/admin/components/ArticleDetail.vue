...
import { getArticleById,createArticle } from '@/api/article'
  ...
  methods: {
    ...
    async fetchData(id) {
      getArticleById(id).then(response => {
        ...
        this.postForm = data
        //根据classId查询对应的名称
        getClassNameById(data.sortClass).then(response => {
          this.postForm.sortClass = getResultRespCode(response).sortClass
        }
        )
        this.postForm.articleStatus = data.articleStatus
