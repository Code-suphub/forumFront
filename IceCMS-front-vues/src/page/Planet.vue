                ...
                <div class="row">
                  <el-aside width="260px">
                    <div class="sidebar-innter widget-ffixed">
                      <section id="b2-widget-circle-info-2" class="mobile-hidden widget b2-widget-circle-info mg-b box b2-radius-aside-left">
                          ...
                          <div class="about-widget">
                            <div class="user-w-announcement">
                              <ul class="planet-aside-ul">
                                <li class="planet-aside-li el-icon-ice-cream-round">
                                  <a href="#">怎么创建圈子？</a>
                                </li>
                                <li class="planet-aside-li el-icon-ice-cream-round">
                                  <a href="#">如何进行沟通？</a>
                    ...
                    <div class="sidebar-innter widget-ffixed">
                      <section id="b2-widget-circle-info-2" class="mobile-hidden widget b2-widget-circle-info mg-b box b2-radius-aside-left-plant">
                        <div class="b2-widget-title">
                                  ...
                                  <div style="margin-left: 10px" class="PlanetListItems">
                                    ...
                                    <div class="PlanetListItem-info">
                                      <div class="PlanetListItem-title">
                                        <a class="planetTitletext" :href="'/planet/' + item.id">
                                          <span>{{ item.name }}</span>
                                        </a>
                                      </div>
                                    </div>
                                  ...
                                  <h1 class="planeth1">
                                    <p>
                                      ...
                                      <span class="mobile-show">
                                        <i class="b2fontb2-arrow-right-s-linepicked"></i>
                                      </span>
                                    </p>
                                    <a target="_blank">
                                      <span>Alextao</span>
...
<!--                                  </div>-->
                                  <div>
                                    <div class="top-card">
                                      <span class="">所有圈子({{this.classlist.length}})</span>
                                    </div>
                              ...
                              <div v-else>
                                <div class="postFormDiv" style="padding: 14px">
                                  <router-link to='/userinfo/index/'>
                                    <div class="avatar">
                                      <el-avatar :size="58" :src="this.user.avatar"></el-avatar>
                                    </div>
                                  </router-link>
                        ...
                        <div class="circle-contenr">
                          <div class="topic-header">
                            <div class="topic-header-left">
                              <router-link :to="userInfoLink(item.userid)">
                                ...
                                <div>
                                  <router-link :to="userInfoLink(item.userid)">
                                    <div class="meta-item post-author">
                                      <a :href="userInfoLink(item.userid)" class="author-name">{{item.author}}</a>
                                    </div>
                                  </router-link>
<!--                                  <div class="topic-user-lv">-->
<!--                                     <p>-->
<!--                                      <span class="lv-icon user-vip b2-vip3"><i style="border-color: #ff8223"></i>-->
<!--                                        <b style="color: #ff8223">永久会员</b>-->
<!--                                      </span>-->
<!--                                    </p>-->
<!--                                    <p>-->
<!--                                      <span class="lv-icon user-lv b2-lv5"><b>荣誉用户</b>-->
<!--                                        <i>lv5</i>-->
<!--                                      </span>-->
<!--                                    </p>-->
<!--                                  </div>-->
                        ...
                        <div class="topic-footer">
                          <div class="topic-footer-left">
                            <button v-if="!item.isLike" @click="likeClickMains(item)" class="planettext ">
                              ...
                              <span class="el-icon-caret-planettext">{{ item.loveNum > 0 ? '\xa0' + item.loveNum + '\xa0\xa0赞' : '赞' }}</span>
                              <b></b>
                            </button>
                            ...
                            <span class="topic-date">
                              <b>
                                <time class="b2timeago" datetime="2021-12-27 21:58:17" itemprop="datePublished">
                              ...
                              <div>
                                ...
                                <div>
                                  <div class="oauth-login-button">
                                    <a href="javascript:void(0)" class="login-weixin">微信登录</a>
                                    <a href="https://api.weibo.com/oauth2/authorize?client_id=2224666867&amp;response_type=code&amp;redirect_uri=https%3A%2F%2F7b2.com%2Fopen%3Ftype%3Dweibo"
                                      class="login-weibo">微博登录</a>
...
export default {
  ...
  watch: {
    $route(to, from) {
      ...
      getAllSquare(this.$route.params.square, this.page, 6).then((response) => {
        ...
        const data = getListRespData(response)
      ...
      getArticleClassByOtherName(this.$route.params.square).then(
        (response) => {
          this.planetInfo = getResultRespCode(response);
        }
      );
    },
    // 如果路由有变化，会再次执行该方法
  ...
  methods: {
    userInfoLink(userId) {
      if ((this.user!==undefined && this.user!=="") && this.user.userId===userId) {
        return '/userInfo';
      } else {
        return `/otheruserinfo/${userId}`;
      }
    },
    refreshPage() {
      setTimeout(() => {
    ...
    setPullDown(item) {
      ...
      if (!item.isShow) {
        getPlanetIdComment(item.id).then((res) => {
          this.comment = getResultRespData(res);
        });
        this.$set(item, "isShow", false);
        item.isShow = !item.isShow;
      } else {
    ...
    getSquare: function () {
      ...
      if (this.$route.params.square) {
        getArticleClassByOtherName(this.$route.params.square).then(
          (response) => {
            this.planetInfo = getResultRespData(response);
          }
        );
        //获取全部评论
        getAllSquare(this.$route.params.square, this.page, 6).then((response) => {
          ...
          this.squaredata = this.squaredata.concat(data)
        });
      } else {
        getArticleClassByOtherName(1).then((response) => {
          this.planetInfo = getListRespData(response);
        });
        //获取全部评论
        getAllSquare(0, this.page, 6).then((response) => {
          const data = getListRespData(response)
          for (let i = 0; i < response.data.data.length; i++) {
            data[i].image = JSON.parse(data[i].image)
          }
    ...
    fetchData() {
      ...
      getSquareClassList().then((res) => {
        this.classlist = getListRespData(res);
      });
    },
    sitmap() {
      if (this.$route.params.square) {
