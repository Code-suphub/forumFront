...
import {getListRespData, getListRespTotal} from "@/utils/respUtils";
export default {
  name: "Home",
  components: { top, foot },
  computed: {
  ...
  methods: {
    a_changeDiscuss() {
      this.a_sortOrder = "post_num"
      this.listLoading = true;
      getArticleSortByCondition(6, this.a_sortOrder).then((resp) => {
        this.list = getListRespData(resp);
    ...
    r_changeRecommend() {
      ...
      getNewResource(10, this.r_sortOrder).then((resp) => {
        this.rlist = getListRespData(resp);
      });
      this.listLoading = false;
    },
    r_changeNews() {
    ...
    getSetting() {
      getCarousel().then((resp) => {
        this.Carousel = getResultRespData(resp);
      });
    },
  },
};
