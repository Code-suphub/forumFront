...
export default {
  ...
  methods: {
    ...
    queryOrderStatus() {
      const user = getUser()
      console.log('查询订单状态')
      updateIntegral(user.userid, this.form.integral, this.orderNo).then(resp => {
        console.log(resp)
    ...
    selectPayTypeLogin(type) {
      ...
      //支付宝支付
        if (this.payOrder.payType === 'alipay') {
          ...
          //调用统一下单接口
            aliPayApi.ftofPayVipIntegralLogin(this.form.integral, user.userid).then(response => {
        ...
        if (this.payOrder.payType === 'wxpay') {
          ...
          //调用统一下单接口
          wxPayApi.nativePayVipIntegralLogin(this.form.integral, user.userid).then(response => {
            console.log(response.data.data.codeUrl)
            this.codeUrl = response.data.data.codeUrl
            this.orderNo = response.data.data.orderNo
    ...
    onSubmit() {
      ...
      if (!this.userJudje) { this.userid = user.userId }
      if (this.userJudje) {
        //未登录提醒登陆
        this.$notify({
          title: '失败',
