  ...
  export default {
    ...
    methods: {
      ...
      queryOrderStatus() {
        const user = getUser()
        console.log('查询订单状态')
        createVip(user.userId, this.payid, this.orderNo).then(resp => {
          console.log(resp)
      ...
      selectPayTypeLogin(type) {
        ...
        if (this.payOrder.payType === 'alipay') {
          ...
          //调用统一下单接口
          const user = getUser()
          aliPayApi.ftofPayForVipLogin(this.form.integral, user.userId, this.payid).then(response => {
        ...
        if (this.payOrder.payType === 'wxpay') {
          ...
          //调用统一下单接口
          const user = getUser()
          wxPayApi.nativePayVipLogin(this.form.integral, user.userId, this.payid).then(response => {
