<template>
  <div class="home">
    ...
    <el-table :data="dataList" style="width: 500px">
      <el-table-column label="头像" align="center">
        <template slot-scope="scope">
          <el-avatar :src="scope.row.followeeAvatar" :size="40"></el-avatar>
        </template>
      </el-table-column>
      <el-table-column label="被关注者" prop="followeeNickname"></el-table-column>
    ...
    <el-dialog title="提示" :visible.sync="dialogVisible" width="30%" @close="handleDialogClose">
      <span>确定要取消关注吗？</span>
...
export default {
  ...
  methods: {
    showDialog(row) {
      ...
      this.currentRow = row; // 保存当前操作的行数据
    },
    handleDialogClose() {
    ...
    confirmUnfollow() {
      // 在这里实现取消关注的逻辑
      console.log('取消关注:', this.currentRow);
      // 假设你通过API调用取消关注
      unfollow(this.currentRow.followerId,this.currentRow.followeeId).then(res=>{
    ...
    getFollowList(){
      followers(this.user.userId).then(res => {
        this.dataList = getListRespData(res)
      })
    },
  }
}
