<template>
  <transition name="slide-fade">
    <div v-if="close" class="chat-container overflow-hidden van-popup van-popup--round van-popup--bottom">
      <div class="chat-header position-relative">评论列表
        <i @click="closeComment()" class="icon-close cursor-pointer" />
      </div>
      <div class="h-full w-full">
        <div id="chatRecord" class="chat-body p-4">
          <div class="in finite-loading-container" data-v-ecb5dde6="">
            <div class="infinite-status-prompt" style="
                ...
                display: none;
              " data-v-ecb5dde6="">
              <i class="loading-default" data-v-4b642ee6="" data-v-ecb5dde6=""/>
            </div>
            <div class="infinite-status-prompt" style="display: none" data-v-ecb5dde6="">
              <div class="mt-10" data-v-ecb5dde6="">
                <img src="static/picture/comments.svg" class="w-80 mb-4" data-v-ecb5dde6=""/>
                <h5 class="mb-6" data-v-ecb5dde6="">还没有评论哦!</h5>
              </div>
            </div>
            <div class="infinite-status-prompt" style="display: none" data-v-ecb5dde6="">
              <div class="mb-3 fs-12 text-muted" data-v-ecb5dde6="">全部数据已加载完毕……</div>
            </div>
            <div class="infinite-status-prompt" style="
                color: #666;
                font-size: 14px;
                ...
                display: none;
              " data-v-ecb5dde6="">
              <div data-v-ecb5dde6="">数据加载失败，
                <button class="btn btn-outline-theme btn-sm" data-v-ecb5dde6="">重试</button>
              </div>
            </div>
          </div>
          <!-- ---------- -->
          <div v-for="(item, id) in comment" :key="id" class="d-flex mb-3">
                    ...
                    <g transform="translate(825.000000, 1100.000000)">
                      ...
                      <g id="Avataaar" stroke-width="1" fill-rule="evenodd" fill="black" mask="url(#mask-2)">
                        <g id="Body" transform="translate(32.000000, 36.000000)">
                          ...
                          <use fill="#AE5D29" xlink:href="#path-silhouette"></use>
                          ...
                          <g id="Top" strokewidth="1" fillrule="evenodd">
                            ...
                            <g id="Top/Short-Hair/Short-Waved" mask='url("#ShortHairShortWaved-mask2")'>
                              <g transform="translate(-1.000000, 0.000000)">
                                ...
                                <use class="CustomColor" stroke="none" fill="var(--avataaar-hair-color)" fillrule="evenodd" href="#ShortHairShortWaved-path1"></use>
                              </g>
                            </g>
                          </g>
                        </svg>
                        ...
                        <svg>
                          <g id="Top/_Resources/Round" fill="none" transform="translate(62.000000, 85.000000)">
                            ...
                            <g id="Lennon-Glasses">
                              <use fill="black" fillopacity="1" filter='url("#filter1")' href="#Round-path1"></use>
                              <use fill="#252C2F" fillrule="evenodd" href="#Round-path1"></use>
                            </g>
                          </g>
                        </svg>
                      </g>
            ...
            <div v-if="item.userId === user.userId">
              <section class="flex-grow">
                <div class="mb-1">
                  <span class="fs-12 mr-1 fw-600 text-darken">{{item.nickName}}</span>
                  <span class="opacity-70 fs-12 text-muted pl-1"
                  ><span>{{ formatDate(item.addTime) }}</span></span
                  >
                </div>
                <!---->
                <div class="message-right mb-1">{{ item.content }}</div>
              </section>
            </div>
            <div v-else>
              <section class="flex-grow">
        ...
        <div class="chat-footer">
          <div>
            <textarea v-model="contentArea" rows="2" placeholder="尽情畅所欲言把"/>
          </div>

          <div class="d-flex align-items-center pl-3 clearfix">
            <a @click="openEmoji()" class="nav-link p-0 cursor-pointer"><i class="icon-smile fs-18"/></a>
            <span>
              <div id="el-popover-4635" role="tooltip" aria-hidden="true" class="el-popover el-popper emoji-popover" style="width: undefinedpx; display: none">
                <!---->
                <div class="macwk-emoji">
                  <div class="d-flex nav">
                    ...
                    <a class="nav-link flex-grow-1 cursor-pointer">
                      <span>
                        <svg xmlns="http://www.w3.org/2000/svg" viewbox="0 0 24 24" width="24" height="24">
                          <path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0m0 22C6.486 22 2 17.514 2 12S6.486 2 12 2s10 4.486 10 10-4.486 10-10 10"/>
                          <path d="M8 7a2 2 0 1 0-.001 3.999A2 2 0 0 0 8 7M16 7a2 2 0 1 0-.001 3.999A2 2 0 0 0 16 7M15.232 15c-.693 1.195-1.87 2-3.349 2-1.477 0-2.655-.805-3.347-2H15m3-2H6a6 6 0 1 0 12 0"/></svg>
                      </span>
                    </a>
                    <a class="nav-link flex-grow-1 cursor-pointer">
                      <span>
                  ...
                  <div class="p-2">
                    <div class="input-group">
                      <input type="text" placeholder="搜索表情……" value="" class="form-control"/>
            ...
            <a href="/Privacy" target="_blank" class="fs-12 ml-5" style="margin-top: -2px">查看隐私政策</a>
            <div class="comment-post flex-grow text-right">
              <span>
                <button @click="sendComment()" class="btn btn-theme btn-sm px-5 pt-1 fs-14" style="border-top-left-radius: 10px">评论</button>
              </span>
            </div>
          </div>
        </div>
...
<script>
import { getAllArticleComment,addArticleComment } from '@/api/articleComment'
import { validEmail } from '@/utils/validate.js'
import { getUser } from '@/utils/userUtils'
import {getListRespData} from "@/utils/respUtils";
import { formatDate } from '@/utils/date.js'
import { VEmojiPicker, emojisDefault, categoriesDefault } from "v-emoji-picker";

export default {
  name: 'comment',
  props: ['articleId', 'theEmoji'],
  components: {
    VEmojiPicker
  },
  setup() {
  ...
  created: function () {
    getAllArticleComment(this.articleId).then(resp => {
      this.comment = getListRespData(resp)
    })
    this.user = getUser()
  },
  data() {
  ...
  methods: {
    ...
    closeComment(val) {
      // console.log("clos")
      this.close = false;
      this.$emit('closeCommentName');
      //closeComment 事件触发后，自动触发 closeComment 事件
    },
    openEmoji() {
      // console.log("open")
      this.$emit('openthecpmmentName');
      //openEmoji 事件触发后，自动触发 openthecpmmentName 事件
    }
  }
}
</script>
