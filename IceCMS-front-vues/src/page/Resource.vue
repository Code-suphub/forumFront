...
import { getArticleSortByCondition } from "@/api/article";
...
import {getListRespData, getResultRespData} from "@/utils/respUtils";
export default {
  ...
  watch: {
    $route: function (to, from) {
      ...
      getArticleSortByCondition(2).then((resp) => {
  ...
  methods: {
    ...
    queryOrderStatusBytrues(resourceid, userid) {
      orderInfoApi.queryOrderStatusBytrue(userid, resourceid).then(response => {
        ...
        if (getResultRespData(response) === 0) {
          this.payJudej = false
        }
        else {
          this.payJudej = true
    ...
    queryOrderStatus() {
      orderInfoApi.queryOrderStatus(this.orderNo).then(response => {
        // console.log('查询订单状态：' + getResultRespCode(response))

        // 支付成功后的页面跳转
        if (getResultRespCode(response) === 0) {
          console.log('清除定时器')
    ...
    selectPayTypeLogin(type) {
      ...
      if (this.payOrder.payType === 'alipay') {
        ...
        aliPayApi.ftofPayLogin(this.$route.params.id, this.userid).then(response => {
          const data  = getResultRespData(response)
    ...
    fetchData(id) {
      ...
      getPrenewsResource(id).then((resp) => {
        const data = getResultRespData(resp)
        this.preResource.title = data.title;
        this.preResource.id = data.id;
        if (data.createTime != null) {
          this.preResource.addTime = data.createTime;
        } else {
          this.preResource.addTime = data.addTime;
        }
      });
      getLastnewsResource(id).then((resp) => {
        const data = getResultRespData(resp)
        this.nextResource.title = data.title;
        this.nextResource.id = data.id;
        if (data.createTime != null) {
          this.nextResource.addTime = data.createTime;
        } else {
          this.nextResource.addTime = data.addTime;
        }
      });
    },
  },
