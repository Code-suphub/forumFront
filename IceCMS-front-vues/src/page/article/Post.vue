...
<!--&lt;!&ndash;                                    <router-link :to="{ path: '/otheruserinfo:', query: { authorId: this.authorId }}" class="author-name">{{ this.author }}</router-link>&ndash;&gt;-->
<!--                                    <router-link :to="{ path: '/otheruserinfo:'+ {{this.authorId }}" class="author-name">{{ this.author }}</router-link>-->
<!--                                  </div>-->
                                  <div class="meta-item post-author"><span class="categorys">分类</span>
                                    <a :href="userInfoLink" class="author-name">{{this.author}}</a>
                                  </div>
                                  <span class="meta-item">{{formatDate(this.addTime)}} </span>
                                  <span class="meta-item"><i class="el-icon-chat-line-square"></i>{{this.commentNum}}</span>
                                  <span class="meta-item">点击 {{this.hits}}</span>
                                  <span class="meta-item"><i class="el-icon-share"></i>分享</span>
                                </div>
                                <!--文章内容显示-->
...
<!--                                              </div>-->
                                            <img class="quote-icon" src="@/static/img/quote-icon.png" alt="">
                                          </div>
                   ...
                   <!-- Tags -->
                        <div class="tags">
                            ...
                            <div class="categories-tags">
                              <a v-for="item in myTag" :key="item.id"  :href="'/post/all/'+ item">{{item}} </a>
                            </div>
                        </div>
                        <!-- Post navigation前后文章跳转 -->
                          ...
                          <router-link :to="'/post/' + preArticle.id">
                            <div class="nav-prev">
                              ...
                              <div class="nav-post">
                                ...
                                <span class="date body-text">{{formatDate(preArticle.addTime)}}</span>
                              </div>
                            </div>
                          </router-link>
                          <router-link :to="'/post/' + nextArticle.id">
                            <div class="nav-next">
                              <div class="nav-button-right">下一篇
                                <span><img src="@/static/img/long-arrow.png" alt="arrow"></span>
                              </div>
                              <div class="nav-post">
                                <h3 class="heading-tertiary">{{nextArticle.title}}</h3>
                                <span class="date body-text">{{formatDate(nextArticle.addTime)}}</span>
                              </div>
                            </div>
                          </router-link>
                        </div><!-- Comments -->
                ...
                <div class="col-lg-3">
                    ...
                    <i class="el-icon-search"></i>
                        ...
                        <div v-for="item in newArticle" :key="item.id" class="recent-post-card">
                          <router-link  :target="_self" :to="'/post/' + item.id">
<!--                          <a href="">-->
                            <div>
...
<!--                                  <br>-->
                                  <span  v-if="item.createTime != null"  class="meta-item"> &nbsp&nbsp{{formatDate(item.createTime)}} </span>
                                  <span  v-else  class="meta-item"> {{formatDate(item.addTime)}} </span>
                                  <span class="meta-item"><i class="el-icon-chat-line-square"></i>{{item.commentNum}} </span>
                                </div>
...
<!--                          </a>-->
                       ...
                       <!-- Widget category -->
                        <div class="widget-categories">
                            ...
                            <div class="categories-tags">
                              ...
                              :href="'/post/all?tag=' + item.tagName" >{{item.tagName}} </a>
                            </div>
                            ...
                            <div  v-for="item in newArticle" :key="item.id" class="widget-category">
                              ...
                              <router-link :to="'/post/' + item.id">
                                <div class="cat-content">
                                  <a href="#"><h4 class="cat-title">{{item.title}}</h4></a>
                                  <div class="cat-meta">
                                    ...
                                    <span class="meta-item comment"><i class="el-icon-chat-line-square"></i>{{item.commentNum}} </span>
                                  </div>
                                </div>
                              </router-link>
                            </div><!-- Cat item -->
                          ...
                          <div class="crayons-article-actions print-hidden">
                            <div style="
                              ...
                              <button @click="loveClick()"
                                ...
                                data-category="like"
                                title="Heart">
                                <span v-if="!lovecheck" class="crayons-reaction__icon crayons-reaction__icon--inactive">
                                 <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" role="img" aria-hidden="true" class="crayons-icon">
                                    <path d="M21.179 12.794l.013.014L12 22l-9.192-9.192.013-.014A6.5 6.5 0 0112 3.64a6.5 6.5 0 019.179 9.154zM4.575 5.383a4.5 4.5 0 000 6.364L12 19.172l7.425-7.425a4.5 4.5 0 10-6.364-6.364L8.818 9.626 7.404 8.21l3.162-3.162a4.5 4.5 0 00-5.99.334l-.001.001z"></path>
                                  </svg>
                               </span>
                                <span v-else class="crayons-reaction__icon crayons-reaction__icon--inactive">
                                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" role="img" aria-hidden="true" class="crayons-icon">
                                    <path fill="red" d="M21.179 12.794l.013.014L12 22l-9.192-9.192.013-.014A6.5 6.5 0 0112 3.64a6.5 6.5 0 019.179 9.154zM4.575 5.383a4.5 4.5 0 000 6.364L12 19.172l7.425-7.425a4.5 4.5 0 10-6.364-6.364L8.818 9.626 7.404 8.21l3.162-3.162a4.5 4.5 0 00-5.99.334l-.001.001z"></path>
                                  </svg>
                                </span>
                                <br>
                                <span v-if="!lovecheck" class="crayons-reaction__count" id="reaction-number-like">{{loveNum}}</span>
                                <span v-else style="color:red" class="crayons-reaction__count" id="reaction-number-like">{{loveNum}}</span>
                              </button>

                              <!--未知的独角兽按钮-->
                              <button style="" id="reaction-butt-unicorn"
                              ...
                              <!--未知的评论按钮-->
                              <button id="reaction-butt-readinglist"
                                ...
                                <br>
                                <span class="crayons-reaction__count" id="reaction-number-readinglist">{{this.commentNum}}</span>
                              </button>
                              ...
                              <div class="align-center m:relative">
                                <button id="article-show-more-button"
                                  ...
                                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" role="img" aria-labelledby="a6r3b1dohi4el9rjyuy41avu2a0sm5bz" aria-hidden="true" class="crayons-icon dropdown-icon">
                                    <title id="a6r3b1dohi4el9rjyuy41avu2a0sm5bz">More...</title>
              ...
              <!-- 右下脚悬浮窗显示评论，后台控制是否显示评论 -->
              <div v-if="!this.glabledata.glableCommentShow">
                  <div v-show="!judgeComment">
                    ...
                    <div class="d-flex
                          ...
                          w-full
                          pl-3">
                        <div class="flex">共 <span class="fs-36 mx-1">{{ this.commentNum }}</span> 条评论 </div>
                        <svg color="inherit"
                          viewbox="0 0 32 32"
                          class="w-32"
                          style="
...
import {getArticleById, getArticleSortByCondition, getPreNewsArticle, getLastNewsArticle, changeArticle} from "@/api/article";
...
import { getAllTag, getTagByList } from "@/api/alltag";

import top from "@/page/components/Top.vue";
import foot from "@/page/components/Foots.vue";
import comment from "@/page/components/Comment.vue";

import { formatDate } from "@/utils/date.js";
import { mapState, mapMutations } from "vuex";
import {formatTime} from "@/utils";
import {getListRespData, getResultRespCode, getResultRespData, getResultRespMsg} from "@/utils/respUtils";
...
export default {
  ...
  computed: {
    ...
    userInfoLink() {
      ...
      if (this.user!==null && this.user!==undefined && this.user.userId===this.authorId) {
        return '/userInfo';
      } else {
        return `/otheruserinfo/${this.authorId}`;
      }
    },
  },
  created() {
    ...
    getArticleCommentNum(this.$route.params.id).then((resp) => {
      this.commentNum = getResultRespData(resp);
    });
    getAllTag().then((resp) => {
        //获取标签
        this.taglist = getListRespData(resp);
      });
    //文章浏览量+1
    changeArticle(this.$route.params.id,"hits").then((resp) => {});
  },
  watch: {
    $route(to, from) {
    //数据回填
  ...
  methods: {
    ...
    loveClick() {
      if (!this.lovecheck) {
        if (this.firstLoveFlag) {
          changeArticle(this.$route.params.id,"love_num").then((resp) => {});
    ...
    fetchData: function (id) {
      getArticleById(id).then((resp) => {
        ...
        this.loveNum = data.loveNum;
        this.author = data.authorName;
        this.authorId = data.authorId;
        if (data.commentDisabled === "true") {
          this.judgeComment = true;
        }
        //目录加载需要延时
        setTimeout(() => {
        ...
        if (data.createTime != null) {
          this.addTime = data.createTime;
        } else {
          this.addTime = data.addTime;
        }
        this.intro = data.intro;
        this.hits = data.hits;
        JSON.parse(data.keyword).forEach((item) => {
          getTagByList(item).then((resp) => {
            this.myTag.push(getResultRespData(resp).tagName);
          });
        });
      });
      getArticleSortByCondition(2, "love_num").then((resp) => {
        this.newArticle = getListRespData(resp);
      });
      getPreNewsArticle(id).then((resp) => {

        if (getResultRespCode(resp) === 200) {
          const data = getResultRespData(resp)
          this.preArticle.title = data.title;
          this.preArticle.id = data.id;
          if (data.createTime != null) {
            this.preArticle.addTime = data.createTime;
          } else {
            this.preArticle.addTime = data.addTime;
          }
        } else {
          this.preArticle.title = getResultRespMsg(resp);
          this.preArticle.addTime = "2024-12-12";
        }
      });
      getLastNewsArticle(id).then((resp) => {
        if (getResultRespCode(resp) === 200) {
          const data = getResultRespData(resp)
          this.nextArticle.title = data.title;
          this.nextArticle.id = data.id;
          if (data.createTime != null) {
            this.nextArticle.addTime = data.createTime;
          } else {
            this.nextArticle.addTime = data.addTime;
          }
        } else {
          this.nextArticle.title = getResultRespMsg(resp);
          this.nextArticle.addTime = "2024-12-12";
        }
      });
    },
  },
