                    ...
                    <div class="flex">
                      ...
                      <div class="app-content-header mb-5">
                        <div class="main-title">
                          <h5 class="fs-24 fw-600 i-con-h-a">全部文章
                            <span class="text-muted fs-13 v-1 ml-1">{{ this.articleCount }}</span>
                          </h5>
                        </div>
                        <nav class="menu menu--macwk——list article-menu flex">
                          <ul class="menu__list">
<!--                            <li @click="getList()" class="menu__item" :class="{ 'menu__item&#45;&#45;current': allIndex }">-->
                            <li @click="findAllArticle()" class="menu__item" :class="{ 'menu__item--current': allIndex }">
                        ...
                        <div>
                          <div role="switch" class="el-switch" style="margin-top: -3px">
                            ...
                            <!---->
                            <div>
                              <el-switch v-model="isTargetJudge" @change="getIsTargetJudge()"
                      ...
                      <div v-for="item in articleList" :key="item.id">
                        <div v-if="item.status.includes('published')">
                          <router-link  :target="target" :to="'/post/' + item.id">
                            <a target="_self" class="post-item white delay-0 list-animation-leftIn">
                              ...
                              <div v-else class="post-item__preview align-items-center d-flex delay-5"
                                   :style="getStyles()">
                              ...
                              <div class="post-item__content">
                                ...
                                <div class="post-meta">
                                  ...
                                  <div class="post-meta__info fw-600">
                                    ...
                                    <span>•</span>
                                    <span v-if="item.createTime != null" v-text="formatDate(item.createTime)"></span>
                                    <span v-else v-text="formatDate(item.addTime)"></span>
                                    <span> • </span><span> 阅读 {{ item.hits }}</span>
                                  </div>
                                </div>
                              </div>
                              <div>
                      ...
                      <!---->
                      <pagination class="app-content-bottom" v-show="total > 0"
                                  :background="false" :total="total"
                                  :page.sync="listQueryClass.page"
                                  :limit.sync="listQueryClass.limit" @pagination="getListByClass"/>
                    </div>
                    <div class="ml-6 w-380 d-md-none d-lg-none d-xl-block">
                        ...
                        <div class="widget-popular-post mt-30">
                             ...
                             <div  v-for="item in newArticle" :key="item.id" class="widget-category">
                               <router-link :target="target" :to="'/post/' + item.id">
                                 ...
                                 <div v-else class="cat-thumb bg-cover" :style="getStyles()">
                                  <h3 class="flex text-center text-white opacity-50">NOPIC</h3>
                                </div>
                              </router-link>
                              <router-link
                            ...
                            >
                                <div class="cat-content">
                                  ...
                                  <span
                                            class="meta-item comment"><i class="el-icon-chat-line-square"></i>{{item.commentNum}} </span></div>
                                </div>
                              </router-link>
                            </div><!-- Cat item -->
                            <!-- Cat item -->
                        ...
                        <div class="widget-categories">
                          ...
                          <h3 class="fs-24 fw-600 mb-0">全部标签</h3>
                            ...
                            <div class="categories-tags">
                              <router-link   v-for="item in taglist" :key="item.id" :target="target" :to="{path:'/post/' + item.id + '/all'}" >
                                {{item.tagName}}
                              </router-link >
                            </div>
                        </div>
                      <div class="d-block siderbar-apps new-post">
                        ...
                        <div
                          ...
                          "
                        >
                          <a target="_blank" :href="item.articleId"
                          ...
                          <div class="d-flex align-items-center">
                            <div class="mr-3">
                              <svg viewBox="0 0 264 280" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" class="w-20 h-20">
                            ...
                            <div class="d-flex flex-column flex-grow-1">
                              <p class="d-flex title fw-400 fs-14 text-ellipsis mb-0">
              ...
              <div id="sidetools"
                ...
                <span>
                  <div role="tooltip" id="el-popover-3962"
                ...
                <span>
                  <div role="tooltip" id="el-popover-1793"
                    aria-hidden="true" class="el-popover el-popper"
                    style="width: 200px; display: none">
                    <!---->
                    <div class="text-center">
...
<script>
import {findAllArticle, getAllArticle, getAllArticleNumber, getArticleSortByCondition,} from "@/api/article";
import Pagination from "@/components/Pagination/index.vue"; // Secondary package based on el-pagination
import {formatDate} from "@/utils/date.js";
import {getArticleClassList} from "@/api/articleClass";
import {getNewArticleComment} from "@/api/articleComment";

import {getAllTag} from "@/api/alltag";

import top from "../components/Top.vue";
import foot from "../components/Foots.vue";
import {getListRespData, getListRespTotal, getResultRespData} from "@/utils/respUtils";
...
export default {
  ...
  methods: {
    findAllArticle: function () {
      findAllArticle(this.allQuery).then((resp) => {
        this.articleList = getListRespData(resp)
      });
    },
    getListByClass() {
      ...
      getAllArticle(this.listQueryClass).then((resp) => {
        //获取文章
        this.articleList = getListRespData(resp);
        this.total = getListRespTotal(resp);
      });
    },
    ...
    getList() {
      getArticleClassList().then((resp) => {
        //获取分类
        this.classlist = getListRespData(resp);
      });
      getAllArticle(this.listQueryClass).then((resp) => {
        //获取文章
        this.articleList = getListRespData(resp);
      ...
      getNewArticleComment(9).then((resp) => {
        //获取最新评论
        this.Newarticlecomment = getListRespData(resp);
      });
      getArticleSortByCondition(2,"hits").then((resp) => {
          //获取热门文章
